...
def spectrogram(
    ...
    fft = torch.fft.rfft(strided_input, 1, norm="forward")  # norm=False, onesided=True)
...
def fbank(
    ...
    mel_energies = mel_energies.to("cuda")
    ...
    if use_log_fbank:
        ...
        mel_energies = torch.max(mel_energies.to(EPSILON.device), EPSILON).log()
    ...
    if use_energy:
        ...
        if htk_compat:
            ...
            mel_energies = torch.cat((signal_log_energy, mel_energies.to(power_spectrum.device)), dim=1)
...
def mfcc(
    ...
    # size (m, num_ceps)
    feature = feature.matmul(dct_matrix.to("cuda"))

    if cepstral_lifter != 0.0:
        # size (1, num_ceps)
        lifter_coeffs = _get_lifter_coeffs(num_ceps, cepstral_lifter).unsqueeze(0).to("cuda")
